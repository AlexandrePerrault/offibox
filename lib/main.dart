...
import 'package:offibox/data/cip_hospitaliers_parser.dart';
import 'utils/gs1_parser.dart';
import 'data/biosimilaires_loader.dart' as bio;
import 'data/generiques.dart' as gen;
import 'data/data_loader_core.dart';
import 'data/data_loader_extra.dart';
...
String normalizeScannedInput(String raw) {
  // Nettoyage des caractères de contrôle (scanner)
  final clean = raw.replaceAll(RegExp(r'[\x00-\x1F]'), '');

  // GS1 sans parenthèses : 01 + 14 chiffres
  final gs1 = RegExp(r'01(\d{14})').firstMatch(clean);
  if (gs1 != null) {
    final gtin14 = gs1.group(1)!;
    return gtin14.substring(1); // → CIP13
  }
...
// 📐 POSITION BARRE
// ===============================
double topOffset = 114;
double leftOffset = 0;       // calculé après layout
...
Map<String, List<SearchResult>> indexByFirst3 = {};
Map<String, List<SearchResult>> indexByCip3   = {};
  ...
  int dmCount = 0;
  int vetoCount = 0;
int _searchSeq = 0; // 🔒 anti concurrence
Map<String, String> biosimilaireByCip = {};
Map<String, String> _generiquePrincepsByCip = {};
Map<String, String> _princepsToGenericName = {};
...
// valeur = liste déjà filtrée + triée
// ======================================================================

final Map<String, List<SearchResult>> _searchCache = {};
int _searchCacheVersion = 0; // invalide le cache si données changent
...
Future<void> _initData() async {
  _cipHospitaliers = await loadCipHospitaliers();
  biosimilaireByCip = await bio.loadBiosimilairesByCip();
...
void initState() {
  ...
  WidgetsBinding.instance.addPostFrameCallback((_) async {
    ...
    setState(() {
     topOffset = initialMargin;
rightAnchor = screenWidth - initialMargin;
    ...
    // 1️⃣ CORE DATA
    // ─────────────────────────
    final coreResults = await loadCoreData(
      cipHospitaliers: _cipHospitaliers,
...
final update = await fetchLastUpdate(AMC_URL);

    if (!mounted) return;

    setState(() {
      allResults = coreResults;
      lastGithubUpdate = update; 
      _buildIndexes(allResults);
      _invalidateSearchCache();
      loading = false;
    });

    debugPrint('🟢 CORE LOADED → ${allResults.length} items');

    // ─────────────────────────
    // 2️⃣ EXTRA DATA (LENT, NON BLOQUANT)
    // ─────────────────────────
  loadExtraData().then((extraResults) {
  if (!mounted) return;

  // 🔒 fusion IMMUTABLE (clé de la stabilité)
...
void _handleScanText(String raw) {
  _scanEndTimer?.cancel();

  // si GS1 → on attend la fin du scan
  if (raw.contains('01')) {
    _scanEndTimer = Timer(const Duration(milliseconds: 60), () {
      _processFullScan(raw);
    });
    return;
  }

  // sinon → recherche directe (tape clavier)
  _filterResults(raw);
}

void _processFullScan(String raw) {
...
void _buildIndexes(List<SearchResult> items) {
  indexByFirst3.clear();
  indexByCip3.clear();

  for (final r in items) {
    // 🔤 LABEL (3 premières lettres)
    if (r.labelNorm.isNotEmpty) {
      final key = r.labelNorm.length >= 3
          ? r.labelNorm.substring(0, 3)
          : r.labelNorm;

      indexByFirst3.putIfAbsent(key, () => []).add(r);
    }

    // 🔢 CIP (3 premiers chiffres)
    if (r.cipNorm.length >= 3) {
      final key = r.cipNorm.substring(0, 3);
      indexByCip3.putIfAbsent(key, () => []).add(r);
    }
  }
}
...
bool _matchItem({
  ...
  final label = r.labelNorm; // déjà normalisé
  final cip   = r.cipNorm;
  ...
  if (r.source == SourceType.bdm) {
    ...
    if (first.length >= 3) {
      if (!label.startsWith(first) && !cip.startsWith(first)) return false;
    }
...
if (r.source == SourceType.dm || r.source == SourceType.veto) {
  final textTokens = tokens.where((t) => !RegExp(r'^\d+$').hasMatch(t)).toList();
  final numTokens  = tokens.where((t) => RegExp(r'^\d+$').hasMatch(t)).toList();

  // CIP strict si numérique pur
  if (numTokens.isNotEmpty && cip.isNotEmpty) {
    if (numTokens.join() == cip) return true;
  }

  // 🔤 au moins UN token texte doit matcher
...
void _filterResults(String q) {
  final int seq = ++_searchSeq; // 🔒 seul le dernier appel gagne
  ...
  final cacheKey = '$query|$_searchCacheVersion';

  // ==========================================================
  // 🧠 CACHE — ignoré juste après chargement EXTRA
  // ==========================================================
  if (extraDataLoaded && !_justLoadedExtra) {
    final cached = _searchCache[cacheKey];
    if (cached != null) {
      if (!mounted || seq != _searchSeq) return;
      setState(() => filteredResults = cached);
      return;
    }
  } else if (!extraDataLoaded) {
    _searchCache.clear();
  }
  ...
  // 🔢 CIP / GTIN complet → tout ouvrir
  if (isNumeric && query.length >= 6) {
    candidates = allResults;
  }

  // 🧩 EXTRA chargés → pas de pré-filtrage
  else if (extraDataLoaded) {
    candidates = allResults;
  }

  // 🔢 CIP partiel
  else if (isNumeric && indexByCip3.isNotEmpty) {
    candidates = indexByCip3[key] ?? allResults;
  }

  // 🔤 Texte classique (BDM majoritaire)
  else if (indexByFirst3.isNotEmpty) {
    candidates = indexByFirst3[key] ?? allResults;
  }
  ...
  // fallback large si pré-filtrage trop restrictif
  ...
  // ==========================================================
  // 📦 ÉCRITURE CACHE (après une vraie recherche complète)
  // ==========================================================
  if (extraDataLoaded) {
  ...
  // 👉 le moteur est maintenant stabilisé
  _justLoadedExtra = false;

  // ==========================================================
  // ✅ commit UI si toujours valide
  // ==========================================================
  if (!mounted || seq != _searchSeq) return;
        ...
        children: [
          ...
          Padding(
            ...
            child: TextField(
              ...
              onChanged: _handleScanText, // ✅ UNIQUE POINT
            ),
          ),
...
Widget? genericBadgeWidget(SearchResult item) {
  final span = genericBadgeSpan(item);
  if (span == null) return null;
  return span.child;
}

Widget? princepsBadgeWidget(SearchResult item) {
  final span = princepsBadgeSpan(item);
  if (span == null) return null;
  return span.child;
}
...
// ============================================================================
// 🧱 LIGNE 1 RÉSULTAT (RICHTEXT SEUL)
// ============================================================================
Widget _resultLine1(
  BuildContext context,
  SearchResult item,
...
if (item.source == SourceType.veto &&
    item.cip13 != null &&
    item.cip13!.isNotEmpty)
  ...[
    const TextSpan(text: '  (GTIN : '),
    TextSpan(
      text: item.cip13!,
    ...
    const TextSpan(text: ')'),
        ...
        // 📝 LIBELLÉ (avec highlight)
        ..._highlightText(
          context,
          item.source == SourceType.amc
              ? 'Mutuelle ${item.label} — code préfectoral : ${item.cip13 ?? ''}'
              : label,
          ...
          WidgetSpan(
            ...
            child: Padding(
              ...
              child: InkWell(
                onTap: () {
                  final tel =
                      item.phone!.replaceAll(RegExp(r'[^0-9]'), '');
                  _openUrl('tel:$tel');
                },
                child: Row(
                  mainAxisSize: MainAxisSize.min,
...
List<InlineSpan> cip13Spans(String cip13) {
  if (cip13.length != 13) {
    return [
      TextSpan(text: ' CIP : $cip13'),
    ];
  }

  final part1 = cip13.substring(0, 5);   // 34009
  final part2 = cip13.substring(5, 12);  // 7 chiffres centraux
  final part3 = cip13.substring(12);     // clé
...
List<InlineSpan> cip13ParensSpans(String cip13) {
  ...
  final part1 = clean.substring(0, 5);   // 34009
  final part2 = clean.substring(5, 12);  // 7 chiffres centraux
  final part3 = clean.substring(12);     // dernier chiffre

  return [
    const TextSpan(text: '  (CIP : '),
    TextSpan(text: part1),
    const TextSpan(text: ' '), // ✅ espace AVANT le gras
    TextSpan(
      text: part2,
      style: const TextStyle(fontWeight: FontWeight.w800),
    ),
    const TextSpan(text: ' '), // ✅ espace APRÈS le gras
    TextSpan(text: part3),
    const TextSpan(text: ')'),
  ];
}
...
Widget _standardLine2(BuildContext context, SearchResult item) {
  final widgets = <Widget>[];

  // 🧬 BIOSIMILAIRE
  if (item.source == SourceType.bdm &&
      item.biosimilaireOf != null &&
      item.biosimilaireOf!.isNotEmpty) {
    widgets.add(
      InkWell(
        onTap: () => _openUrl(
        ...
        child: _badgeContainer(
          text: 'BIOSIMILAIRE DE ${item.biosimilaireOf}',
        ),
      ),
    );
  }
  ...
  // 🧬 GÉNÉRIQUE / PRINCEPS
  final generic = genericBadgeWidget(item);
  if (generic != null) widgets.add(generic);

  final princeps = princepsBadgeWidget(item);
  if (princeps != null) widgets.add(princeps);
  ...
  if ((item.source == SourceType.bdm ||
    ...
    widgets.add(
      HoverPillButton(
        ...
        onTap: () {
          Clipboard.setData(ClipboardData(text: item.cip13!));
          _showCopyToast();
        },
      ),
    );
...
Widget _amcLine2(BuildContext context, SearchResult item) {
  return Padding(
    padding: const EdgeInsets.only(top: 4),
    child: Wrap(
      spacing: 6,
      children: [
        if (item.cip13 != null && item.cip13!.isNotEmpty)
          HoverPillButton(
            label: 'COPIER LE CODE',
            icon: Icons.copy,
            tooltip: 'Copier le code préfectoral',
            onTap: () {
              Clipboard.setData(
                ClipboardData(text: item.cip13!),
              );
              _showCopyToast();
            },
          ),
...
// ✅ BDM : enlever le préfixe "CIP : 3400xxxxxxxxxxxx"
if (item.source == SourceType.bdm) {
  displayLabel = displayLabel.replaceFirst(
    RegExp(r'^\s*CIP\s*:\s*\d{13}\s*', caseSensitive: false),
    '',
  );
}
        ...
        final label = item.source == SourceType.amc
            ? 'Mutuelle ${item.label} — code préfectoral : ${item.cip13 ?? ''}'
            : displayLabel
                .replaceAll(RegExp(r'\(BIOSIMILAIRE\)', caseSensitive: false), '')
                .replaceAll(
                  RegExp(r'\s*–\s*biosimilaire de [^–]+$', caseSensitive: false),
                  '',
                )
          ...
          child: ListTile(
            ...
            title: Column(
              ...
              children: [
                ...
                // ✅ ligne 2 AMC
                if (item.source == SourceType.amc)
                  _amcLine2(context, item),

                // ✅ ligne 2 autres sources
                if (item.source != SourceType.amc &&
...
List<InlineSpan> medicineIconSpans(SearchResult item) {
 ...
 // 🟥 HOP — usage hospitalier / CIP hospitalier
if (item.hospitalOnly == true) {
  spans.add(
    hopSquareSpan(
...
WidgetSpan biosimilairesListBadgeSpan({required VoidCallback onTap}) {
  return WidgetSpan(
    alignment: PlaceholderAlignment.middle,
    child: Padding(
      padding: const EdgeInsets.only(right: 6),
...
Widget _badgeContainer({
  ...
  Color background = const Color(0xFFF3E8FF),
...
InlineSpan plusInfosInlineSpan({
  required VoidCallback onTap,
}) {
  return WidgetSpan(
    alignment: PlaceholderAlignment.middle,
